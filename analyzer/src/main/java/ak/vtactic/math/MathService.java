...
public class MathService {
	...
	public DiscreteProbDensity deconvReg(DiscreteProbDensity a,DiscreteProbDensity b) {
		DiscreteProbDensity result = new DiscreteProbDensity(a.numSlots,a.min,a.max,a.offset);		
		try {
			MWNumericArray mResult = (MWNumericArray)conv.deconv_reg(1,
				from(a.getPdf()),
				from(b.getPdf()))[0];
			result.setPdf(mResult.getDoubleData());
			result.raw = null;
	...
	public DiscreteProbDensity deconvLucy(DiscreteProbDensity a,DiscreteProbDensity b) {
		DiscreteProbDensity result = new DiscreteProbDensity(a.numSlots,a.min,a.max,a.offset);		
		try {
			MWNumericArray mResult = (MWNumericArray)conv.deconv_lucy(1,
				from(a.getPdf()),
				from(b.getPdf()))[0];
			result.setPdf(mResult.getDoubleData());
			result.raw = null;
	...
	*/
	...
	public static void main(String[] args) {
		...
		service.matlabService = new MatlabService();
		service.init();
		...
		DiscreteProbDensity shrink = service.shrink(result, 2, 0.5);
		for (double d:shrink.pdf) {
			System.out.print(d+" ");
		}
		System.out.println();
		System.out.println(service.filter(result, shrink));
